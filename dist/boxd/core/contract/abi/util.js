...
var parseTypeArray = function (type) {
    var tmp = type.match(/(.*)\[(.*?)\]$/);
    if (tmp) {
...
var parseNumber = function (arg) {
    var type = typeof arg;
    if (type === 'string') {
        if (util_1.default.isHexPrefixed(arg)) {
            return new bn_js_1.default(util_1.default.stripHexPrefix(arg), 16);
        }
        else {
...
// Parse N from type<N>
var parseTypeN = function (type) {
    var typesize = /^\D+(\d+).*$/.exec(type);
    return typesize ? parseInt(typesize[1], 10) : null;
};
var setLengthLeft = function (msg, length, right) {
    if (right === void 0) {
        right = false;
    }
...
var parseTypeNxM = function (type) {
...
// Is a type dynamic?
var isDynamic = function (type) {
    // FIXME: handle all types? I don't think anything is missing now
    return (type === 'string' || type === 'bytes' || parseTypeArray(type) === 'dynamic');
};
// Encodes a single item (can be dynamic array)
// @returns: Buffer
var encodeSingle = function (type, arg) {
    var size, num, ret, i;
    if (type === 'address') {
...
var elementaryName = function (name) {
...
var decodeSingle = function (parsedType, data, offset) {
    ...
    if (parsedType.name === 'address') {
        ...
        return (decodeSingle(parsedType.rawType, data, offset).toString() ===
            new bn_js_1.default(1).toString());
    }
        ...
        num = new bn_js_1.default(data.slice(offset, offset + 32), 16, 'be');
        ...
        num = new bn_js_1.default(data.slice(offset, offset + 32), 16, 'be').fromTwos(256);
        if (num.bitLength() > parsedType.size) {
        ...
        size = new bn_js_1.default(2).pow(new bn_js_1.default(parsedType.size[1]));
